...
results from 2018-19 is provided in *Appendix* 5.1D.
...
distortion of the measurement system. All items included in the 2018-19
operational assessment are represented. Point measure correlations ranged from
0.24 to 0.76 in ELA, 0.04 to 0.73 in Math, to 0.20 to 0.76 in Science. All data
visualizations were conducted with *ggplot* in the *tidyverse* package (Wickham, H.,
2017).

```{r ifiles}
ifiles <- dir_ls(here::here("data", "ifiles19"))
...
each Grade 4, 5, 7, and 11 was above 2.0 and will be removed. One item in each
of Grades 3, 5, and 7 in Math and one item in each of Grades 5, and 11 in
Science will also be removed.

```{r outfit}
outfit_tbls <- ifiles %>% 
  group_by(content_area, grade) %>% 
  summarize(mean_outfit = mean(outfit),
            min_outfit  = min(outfit),
            max_outfit  = max(outfit)) %>% 
  rename(Grade = grade,
         Mean  = mean_outfit, 
         Min   = min_outfit,
         Max   = max_outfit) %>% 
...
Overall results are largely consistent with 2017-18, with approximately 30-40%
(exception Grade 12 ELA and Science with nearly 70% ) of students with
significant cognitive disabilities achieving proficiency across grades and
content areas. The data visualizations presented below were conducted with
...
nested_perc <- ode %>% 
  ...
  nest(-content_area) %>% 
  mutate(content_area = c("English/Language Arts", "Math", "Reading", 
                          "Science", "Writing"),
...
kids <- df %>% 
  group_by(SSID, grade, form) %>% 
  summarize(tot_score = sum(score)) %>% 
  ungroup()

kids %>%
  filter(form == "ELA_G3", "ELA_G4", "ELA_G5", "ELA_G6", "ELA_G7", 
                  "ELA_G8", "ELA_G11", "ELA_G12") %>%
  count(grade)
...
correlations ranging from 0.61 to 0.98.
...
cors <- ode %>% 
  ...
  mutate(data = map(data, ~remove_empty(., "cols")),
         cor  = map(data, ~corrr::correlate(.) %>% corrr::shave()),
...
correlations between ELA and Reading, as expected, from .96 to .98. Writing is
correlated with ELA from .92 to .96 and with reading from .84 to .88.
