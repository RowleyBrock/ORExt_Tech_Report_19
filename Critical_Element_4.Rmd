...
The following two tables (Table 1 and Table 2) display the percentage of
...
Writing item promps are use during the QT/QA proficiency assessment
...
reliabilities were quite high, ranging from .76 to .92 across the three content
areas and grades. Removing Grade 12 administrations, total test reliability
improves, ranging from .85 to .92. Total test reliability for ELA ranged from
.85 to .92 (Reading from .76 to .88 and Writing from .77 to 85); Math ranged
from .87 to .90; andScience ranged from .84 to .88. Each table below provides
the content area, grade, and total test marginal reliabilities. All test forms
were composed of 36 operational and 12 embedded field-test items (48 items
total).
...
The test information functions displayed below indicate that the scales
exhibit a reliability greater than or equal to .80 for all proficient-level
cutscores (Levels 3 and 4).
...
Math demonstrate incrementally increasing growth and test demands across Grades
3-8, with the exception of Grade 5 Math. The Grade 5 Math assessment is proving
to be more difficult this year, clearly more elaboration of this effort is
needed to address its location on the TCC. Grade 11 and all Science tests are not
vertically scaled; TCCs are thus not presented for Grade 11 or Science. All
Rasch model scaling, as well as the data visualizations for the TCCs were
conducted in the R software 3.3.2 environment (R Core Team, 2016) using the
r2Winsteps package (Anderson, 2015).
\FloatBarrier
![](tccs/ela_tccs.pdf)
![](tccs/math_tccs.pdf)
\clearpage
...
The average standard error of measurment (SEM) associated with each proficiency
level cut score for 2018-19 student data are presented in the table below,
supported by a KEY. The SEM decreased in almost all cases compared to last year,
suggesting that the measures are more reliable when student eligibility is more
strictly controlled. See Section 4.2 below for means and standard deviations by
grade and subject area. <br> KEY: AMO = Annual Measurable Objective, where: Level 1 =
Does Not Yet Meet (not included as the lowest level of proficiency), Level 2 =
Nearly Meets, Level 3	= Meets, and Level 4	= Exceeds.
...
likelihood that a student was appropriately classified as proficient (Annual
Measurable Objective; AMO = 3 or 4) or not proficient (AMO = 1 or 2) and the
likelihood that the student would be classified in the same AMO category given
an additional test administration (Accuracy and Consistency, respectively). The
...
Within the White/Non-White analysis, XX out of 11 items flagged as "C" level
items privileged Non-White test participants in ELA, XX out of 7 privileged
Non-White test participants in Mathematics, and XX out of 4 privileged Non-White
test participants in Science. Overall, DIF flagging bases on race was relatively
balanced, with XX privileging students who were Non-White and XX privileging
...
In terms of the Male/Female analyses, XX out of 14 items flagged as "C" level
items privileged Females in ELA, XX out of 8 flagged items privileged Females in
Mathematics, and XX out of 12 flagged items privileged Females in Science.
Overall, DIF flagging based on sex was relatively balanced, with XX privileging
Females and XX privileging Males.
...
The full race and ethnic demographics for students taking the ORExt are reported
below. Students ethnicity/race was reported in seven categories: (a) American
Indian/Alaskan Native, (b) Asian, (c) Black or African-American, (d) Hispanic,
(e) Multiethnic, (f) Pacific Islander, or (g) White. The majority of students
were reported as White (50% to 57%) or Hispanic (22% to 28%). These results are
largely consistent with the demographics reported for Oregon's general
assessments, though percentages taking the ORExt are slightly higher for most
students of color and generally lower for students who are Asian or White (see
*Appendix* 4.2).
...
Intellectual Disability (ID; ranging from about 32% to 53% across grade and
content areas), students with Autism Spectrum Disorder (ASD; 28% to 37%),
followed by students with Other Health Impairment (OHI; 8% to 18%). ODE policy
...
total n-sizes within each grade level displayed below.
...
The following rellists the number key for **Elibibility Codes** for identified student exceptionalities:
